 ...
 * Findings Service
 *
 * API client for findings domain.
...
const rawFindingsService = {
  /**
   * List Findings
   */
  async getFinding(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/findings` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Raise Finding
   */
  async getFinding(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/findings`;
   ...
   */
  async updateRemediation(findingId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/findings/${findingId}/remediation`;
...
export const findingsService = makeService(rawFindingsService, "findings");
