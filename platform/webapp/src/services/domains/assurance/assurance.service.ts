 ...
 * Assurance Service
 *
 * API client for assurance domain.
...
const rawAssuranceService = {
   ...
   */
  async getStatement(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/assurance/statements` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getStatement(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/assurance/statements`;
   ...
   */
  async getExport(statementId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/assurance/statements/${statementId}/exports`;
