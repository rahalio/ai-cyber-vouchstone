 ...
 * Attribution Service
 *
 * API client for attribution domain.
...
const rawAttributionService = {
  /**
   * Records who discovered a confirmed successful attack. Captured at intake by the incident process,...
   */
  async getAttribution(incidentId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/incidents/${incidentId}/attribution`;
   ...
   */
  async getDiscoveryShare(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/attribution/discovery-share` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getDetectionInterval(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/attribution/detection-intervals` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const attributionService = makeService(rawAttributionService, "attribution");
