 ...
 * Exercises Service
 *
 * API client for exercises domain.
...
const rawExercisesService = {
  /**
   * List Emulation Exercises
   */
  async getExercise(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/exercises` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getExercise(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/exercises`;
   ...
   */
  async getOutcome(exerciseId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/exercises/${exerciseId}/outcome`;
...
export const exercisesService = makeService(rawExercisesService, "exercises");
