...
const rawAssetsService = {
  /**
   * Compartmented. Callers see only assets within their attestation or role scope.
   */
  async getAsset(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/assets` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createAsset(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/assets`;
   ...
   */
  async getAttestation(assetId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/assets/${assetId}/attestation`;
...
export const assetsService = makeService(rawAssetsService, "assets");
